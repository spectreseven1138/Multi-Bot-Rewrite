...
class Music(commands.Cog):
    ...
    class Video:
        ...
        def __init__(self, videoid, download, voice):
            ...
            self.ytdl_opts = {
                ...
                "outtmpl": 'music_download/' + self.videoid,
            }
        ...
        def play(self):
            ...
            if self.videoid in os.listdir("music_download"):
                self.voice.play(discord.FFmpegPCMAudio(source="music_download/" + self.videoid))
            else:
    ...
    @commands.command(brief=str({"type": None, "syntax": "lyrics <song name>", "examples": ["lyrics burn my dread"]}),
                      help="Displays the lyrics for a specified song from Genius.com")
    async def lyrics(self, ctx, *, song_name=None):
        ...
        try:
            ...
            await ctx.send(utils.format_message(ctx, "No lyrics exist for '" + song_name + "' on www.genius.com"))
            return
