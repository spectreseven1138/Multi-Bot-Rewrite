...
class Tasks(commands.Cog):
    ...
    class Category:
        ...
        async def load_tasks(self, config, client):
            ...
            if type(self.guild) is discord.user.User:
                ...
                user = cogs.users.get_user(self.guild.id, config)
    ...
    async def deletetask(self, ctx, *, data):
        ...
        await self.check_key_created(ctx, cogs.users.get_user(ctx.message.author.id, self.Config))
    ...
    async def addtask(self, ctx, *, input_text=None):
        ...
        if utils.is_dmchannel(ctx):
            ...
            user = cogs.users.get_user(guild_id, self.Config)
